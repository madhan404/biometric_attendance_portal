...
const LeaveApprovalStatus = () => {
  ...
  // Define all leave types with their icons and colors
  const leaveTypes = [
    { value: "all", label: "All Types", icon: <EventAvailable /> },
    { value: "sick", label: "Sick Leave", icon: <LocalHospital /> },
    { value: "symposium", label: "Symposium", icon: <Event /> },
    { value: "conference", label: "Conference", icon: <Event /> },
  ...
  const getLeaveTypeIcon = (type) => {
    const foundType = leaveTypes.find(t => t.value.toLowerCase() === type?.toLowerCase());
      ...
      if (Array.isArray(data)) {
        const formattedRequests = data.map((request) => {
          const stages = [
            {
              stage: "Mentor",
              status: request.mentor_approval || "pending",
            },
            {
              stage: "Class Advisor",
              status: request.class_advisor_approval || "pending",
            },
            { 
              stage: "HoD", 
  ...
  const renderEmptyState = () => (
    <LeaveRequestCard sx={{ 
      ...
      {(filterStatus !== "all" || filterType !== "all" || searchTerm) ? (
        ...
        <Chip 
          label="Submit a leave request to get started" 
          color="primary" 
          variant="outlined"
        />
      )}
                        ...
                        <Stack direction="row" alignItems="center" spacing={2} sx={{ overflow: 'hidden' }}>
                          ...
                          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 0.5, sm: 2 }}>
                            <CompactInfoItem
                              ...
                              tooltip={`${leaveData.request_type || "Leave"} Request`}
                            />
                            <CompactInfoItem
                              icon={<CalendarToday />}
                              label="Dates"
                        ...
                        <Box mt={3}>
                          <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
                            ...
                            <Button
                              ...
                              download={`leave_request_${leaveData.request_id}.pdf`}
                            >
                              Download
                            </Button>
                          </Stack>
