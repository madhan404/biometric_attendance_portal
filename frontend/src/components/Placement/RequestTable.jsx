...
const PlacementOfficerRequestManagement = () => {
  ...
  // Define request types specific to placement officer
  const requestTypes = [
  ...
  const fetchLeaveRequests = async () => {
    ...
    try {
      ...
      if (response.data?.requests) {
        // Filter only internship and training requests
        const placementRequests = response.data.requests.filter(
          request => request.request_type === "internship" || request.request_type === "training"
        );
        
        const formattedRequests = placementRequests.map((request) => {
          const stages = [
            {
              stage: "Mentor",
        ...
        setError("Failed to fetch internship/training requests.");
      }
    } catch (err) {
      setError(err.message || "Error fetching internship/training requests.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredRequests = useMemo(() => {
    return requests.filter(request => {
      const matchesSearch = request.student_name?.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          ...
                          request.sin_number?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleRequestAction = async (requestId, action) => {
    try {
      ...
      setNotification({
        open: true,
        message: `Request ${action === "Completed" ? "approved" : "rejected"
          } successfully`,
      ...
      setNotification({
        open: true,
        message: `Failed to ${action === "Completed" ? "approve" : "reject"
          } request`,
        severity: "error",
      });
    } finally {
    ...
    <LeaveRequestCard sx={{ 
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        {filterStatus !== "all" || filterType !== "all" || searchTerm
          ...
          : "There are currently no internship/training requests for your approval."}
      </Typography>
      {(filterStatus !== "all" || filterType !== "all" || searchTerm) ? (
        <Button
          variant="outlined"
        ...
        <>
              ...
              <Grid item xs={12} md={6}>
                <Box sx={{ mb: 3 }}>
                  <Typography variant="subtitle1" color="text.secondary" sx={{ mb: 2, fontWeight: 600 }}>
                    {selectedRequest.request_type === "internship" ? "Internship" : "Training"} Details
                  </Typography>
                  <Stack spacing={2}>
                    <CompactInfoItem
                      icon={<Business />}
                      label="Company"
                      value={selectedRequest.sin_number || "Not specified"}
                    />
                    
                    <CompactInfoItem
                      icon={<School />}
                      label="Location"
                      value={selectedRequest.location || "Not specified"}
                    />
                    ...
                    <CompactInfoItem
                      icon={<AccessTime />}
                      label="Duration"
                      value={`${selectedRequest.internship_duration || selectedRequest.training_duration || 0} weeks`}
                    />
                    ...
                    {selectedRequest.request_type === "internship" && (
                      <CompactInfoItem
                        icon={<Paid />}
                        label="Stipend"
                        value={selectedRequest.stipend || "Not specified"}
                      />
                    )}
                ...
                {(selectedRequest.pdf_path || selectedRequest.company_approval_letter) && (
                  <Box>
                    <Typography variant="subtitle1" color="text.secondary" sx={{ mb: 2, fontWeight: 600 }}>
                      Attachments
                    </Typography>
                    <Stack direction="row" spacing={2}>
                      {selectedRequest.pdf_path && (
                        ...
                        >
                          {selectedRequest.request_type === "internship" ? "Offer Letter" : "Training Letter"}
                        </Button>
                      )}
                      {selectedRequest.company_approval_letter && (
                        <Button
                          variant="outlined"
                          startIcon={<PictureAsPdf />}
                          onClick={() => handleFilePreview(selectedRequest.company_approval_letter)}
                          size={isMobile ? "small" : "medium"}
                        >
                          Company Approval
                        </Button>
          ...
          <DialogActions sx={{ p: 2 }}>
            {selectedRequest.approvals.hod_approval === "approved" &&
              selectedRequest.approvals.placement_officer_approval === "pending" && (
              <>
                <Button
                  color="error"
                  variant="outlined"
    ...
    <Box sx={{ maxWidth: 1450, margin: "auto", p: isMobile ? 1 : 2 }}>
      <Box sx={{ textAlign: "center", mb: 4, mt: -2 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Review and approve student internship/training requests
        </Typography>
      </Box>
        ...
        <>
          <Stack spacing={2}>
            {paginatedRequests.map((request) => {
              ...
              const overallStatus = request.stages.some(
                (stage) => stage.status.toLowerCase() === "rejected"
              )
                ? "rejected"
                : request.stages.every(
                      ...
                      <Grid container spacing={2} sx={{ mb: 2 }}>
                        <Grid item xs={12} md={6}>
                          ...
                          <CompactInfoItem
                            icon={<School />}
                            label="YESR"
                            value={request.year || "Not specified"}
                          />
                          {isSmallMobile && (
                        ...
                        <Grid item xs={12} md={6}>
                          <CompactInfoItem
                            icon={<AccessTime />}
                            label="Duration"
                            value={`${request.internship_duration || request.training_duration || 0} weeks`}
                          />
                          <CompactInfoItem
                            icon={<CalendarToday />}
                            label="Submitted"
