...
const ClassAdvisorRequestManagement = () => {
  ...
  const fetchLeaveRequests = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/classadvisor/students/${user.sin_number}`
      );

      if (response.data?.students) {
        // Debug log to check photo data
        //console.log('Student photo data:', response.data.students[0]?.photo);
        
        // Flatten all leave requests from all students into a single array
        const allRequests = response.data.students.flatMap(student => {
          console.log('Processing student photo:', student.photo); // Debug log
          return student.leave_requests.map(request => ({
            ...request,
  ...
  const getOverallStatus = (request) => {
    ...
    const classAdvisorStatus = request.approvals.class_advisor_approval;
    
    if (classAdvisorStatus === 'rejected') {
      return 'rejected';
    }
    
    if (classAdvisorStatus === 'approved') {
      return 'approved';
    }
  ...
  const handleRequestAction = async (requestId, action) => {
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/classadvisor/approve-request`,
        {
          request_id: requestId,
          action: action === "Completed" ? "approve" : "reject",
      ...
      setNotification({
        open: true,
        message: error.response?.data?.message || `Failed to ${action === "Completed" ? "approve" : "reject"} request`,
        severity: "error",
      });
    } finally {
      setApproving(false);
      ...
      {selectedRequest && (
        <>
          <DialogTitle sx={{ 
            ...
            <Avatar
              sx={{
                ...
                backgroundImage: selectedRequest.student_photo ? `data:image/jpeg;base64,${selectedRequest.student_photo}` : 'none',
                backgroundSize: 'cover',
                backgroundPosition: 'center'
              }}
              src={selectedRequest.student_photo ? `data:image/jpeg;base64,${selectedRequest.student_photo}` : undefined}
            >
          ...
          <DialogActions sx={{ p: 2 }}>
            {selectedRequest.approvals.class_advisor_approval === "pending" && (
              <>
                <Button
                  color="error"
                  variant="outlined"
  ...
  return (
    <Box sx={{ 
      ...
      {filteredRequests.length === 0 ? (
        ...
        <>
          <Stack spacing={2} sx={{ ml: isMobile ? 0 : 35 }}>
                          ...
                          >
                            <Avatar
                              sx={{
                                ...
                                backgroundImage: request.student_photo ? `data:image/jpeg;base64,${request.student_photo}` : 'none',
                                backgroundSize: 'cover',
                                backgroundPosition: 'center'
                              }}
                              src={request.student_photo ? `data:image/jpeg;base64,${request.student_photo}` : undefined}
                            >
