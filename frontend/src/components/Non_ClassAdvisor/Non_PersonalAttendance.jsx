...
// Custom components to break down the large file
const SummaryCards = ({ overallData, theme, viewMode }) => {
          ...
          <Card sx={{ 
            ...
            <CardContent>
              ...
              <Typography variant="body2" sx={{ mt: 1 }}>
                {overallData?.presentDays} of {overallData?.totalWorkingDays} days
              </Typography>
            </CardContent>
          </Card>
        </Grid>
    ...
    <Box>
      ...
      <Paper sx={{ p: 3, mb: 3, borderRadius: 3, boxShadow: 3 }}>
        <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bold', color: theme.palette.primary.dark }}>
          Yearly Summary ({monthlyDisplayData.year})
        </Typography>
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            <Card variant="outlined" sx={{ p: 3, borderRadius: 2, height: "250px" }}>
              <Typography variant="subtitle1" gutterBottom sx={{ fontWeight: 'bold', mb: 2 }}>
                Attendance Overview
              </Typography>
              <Grid container spacing={2}>
                {[
                  { label: 'Present Days', value: monthlyDisplayData.overallStatistics?.totalPresentDays || 0, color: green },
                  { label: 'Absent Days', value: monthlyDisplayData.overallStatistics?.totalAbsentDays || 0, color: red },
                  { label: 'OD Days', value: monthlyDisplayData.overallStatistics?.totalODDays || 0, color: purple },
                  { label: 'Permission Days', value: monthlyDisplayData.overallStatistics?.totalPermissionDays || 0, color: orange }
                ].map((item, index) => (
                  <Grid item xs={6} key={index}>
                    <Box sx={{ 
                      ...
                      borderRadius: 2,
                      borderLeft: `4px solid ${item.color[500]}`
                    }}>
          ...
          <Grid item xs={12} md={6}>
            <Card variant="outlined" sx={{ p: 3, borderRadius: 2, height: '100%' , width: '100%', mr:15}}>
              <Typography variant="subtitle1" gutterBottom sx={{ fontWeight: 'bold', mb: 2 }}>
                Overall Performance
              </Typography>
              <Box sx={{ textAlign: 'center', mb: 3}}>
                <Typography variant="h4" sx={{ fontWeight: 'bold', color: theme.palette.primary.main }}>
                  {monthlyDisplayData.overallStatistics?.overallPercentage || '0.00'}%
                </Typography>
                <Typography variant="subtitle2" color="text.secondary">
              ...
              <LinearProgress 
                variant="determinate" 
                value={parseFloat(monthlyDisplayData.overallStatistics?.overallPercentage) || 0}
                sx={{ 
                  height: 12, 
                  borderRadius: 6,
  ...
  const fetchOverallAttendance = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to fetch overall attendance data');
      }
    } catch (error) {
      console.error('Error fetching overall attendance:', error);
      showSnackbar(error.message || 'Failed to refresh overall attendance data', 'error');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const fetchDailyAttendance = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to fetch daily attendance data');
      }
    } catch (error) {
      console.error('Error fetching daily attendance:', error);
      showSnackbar(error.message || 'Failed to refresh daily attendance data', 'error');
    } finally {
      setLoading(false);
    }
  }, [startDate, endDate]);
  ...
  const fetchWeeklyAttendance = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to fetch weekly attendance data');
      }
    } catch (error) {
      console.error('Error fetching weekly attendance:', error);
      showSnackbar(error.message || 'Failed to refresh weekly attendance data', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const fetchMonthlyAttendance = useCallback(async () => {
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to fetch monthly attendance data');
      }
    } catch (error) {
      console.error('Error fetching monthly attendance:', error);
      showSnackbar(error.message || 'Failed to refresh monthly attendance data', 'error');
    } finally {
      setLoading(false);
    }
  }, [selectedYear]);
  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <Box sx={{ flexGrow: 1, p: isMobile ? 2 : 3, overflowY: 'auto' }}>
        ...
        <SummaryCards overallData={overallData} theme={theme} viewMode={viewMode} />
