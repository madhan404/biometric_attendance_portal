...
const getChartData = (period, selectedDate) => {
  ...
  switch (period) {
    ...
    case "yearly":
      return [
        { label: `${year-3}`, Present: 42, Absent: 5, OD: 2, Permission: 1 },
        { label: `${year-2}`, Present: 40, Absent: 7, OD: 2, Permission: 1 },
        { label: `${year-1}`, Present: 43, Absent: 4, OD: 2, Permission: 1 },
        { label: `${year}`, Present: 41, Absent: 6, OD: 2, Permission: 1 },
      ];
    default:
      return [];
  }
...
const StaffAttendanceDashboard = () => {
  ...
  const TimeSeriesChart = () => {
    ...
    const pieData = [
      { name: 'Present', value: viewMode === 'daily' ? stats.present : stats.presentPercentage, color: COLORS.present },
      { name: 'Absent', value: viewMode === 'daily' ? stats.absent : stats.absentPercentage, color: COLORS.absent },
      { name: 'OD', value: viewMode === 'daily' ? stats.od : stats.odPercentage, color: COLORS.od },
      { name: 'Permission', value: viewMode === 'daily' ? stats.permission : stats.permissionPercentage, color: COLORS.permission }
    ];
