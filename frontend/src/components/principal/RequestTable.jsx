...
// Styled components (same as in ClassAdvisorRequestManagement)
...
const PrincipalRequestManagement = () => {
  const theme = useTheme();
  ...
  const requestTypes = [
    ...
    { value: "sick leave", label: "Sick Leave", icon: <LocalHospital /> },
    ...
    { value: "cultural fest", label: "Cultural Fest", icon: <EmojiEvents /> },
    { value: "sports", label: "Sports", icon: <EmojiEvents /> },
    { value: "internship", label: "Internship", icon: <Work /> },
    { value: "training", label: "Training", icon: <CastForEducation /> },
  ...
  const fetchLeaveRequests = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/staffs-std-leavests`,
        {
      ...
      if (response.data?.requests) {
        const formattedRequests = response.data.requests.map((request) => {
          const stages = [
            {
              stage: "Mentor",
              status: request.approvals.mentor_approval || "pending",
            },
            {
              stage: "Class Advisor",
              status: request.approvals.class_advisor_approval || "pending",
            },
            { 
              stage: "HoD", 
            ...
            },
            {
              ...
              status: request.approvals.placement_officer_approval || "pending",
            },
            {
              stage: "Principal",
  ...
  const filteredRequests = useMemo(() => {
    return requests.filter(request => {
      ...
      const matchesStatus = filterStatus === "all" || 
                          (filterStatus === "approved" && request.approvals.principal_approval === "approved") ||
                          (filterStatus === "pending" && request.approvals.principal_approval === "pending" &&
                           (request.request_type === "internship" 
                            ? request.approvals.placement_officer_approval === "approved"
                            : request.approvals.hod_approval === "approved")) ||
                          (filterStatus === "rejected" && request.approvals.principal_approval === "rejected");
          ...
          request.request_id === requestId
            ? {
                ...
                approvals: {
                  ...
                  principal_approver: {
                    name: response.data.approver_name,
                    sin: response.data.approver_sin,
      ...
      {selectedRequest && (
        <>
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                <Typography variant="subtitle2">
                  ...
                  <Box component="span" sx={{ color: 'text.primary' }}>{selectedRequest.email}</Box>
                </Typography>
                <Typography variant="subtitle2">
                  <Box component="span" sx={{ fontWeight: 500, color: 'text.secondary' }}>Phone: </Box>
                  <Box component="span" sx={{ color: 'text.primary' }}>{selectedRequest.phone}</Box>
                </Typography>
                <Typography variant="subtitle2">
                  <Box component="span" sx={{ fontWeight: 500, color: 'text.secondary' }}>Parent Phone: </Box>
                  <Box component="span" sx={{ color: 'text.primary' }}>{selectedRequest.parent_phone}</Box>
                </Typography>
                <Typography variant="subtitle2">
                  <Box component="span" sx={{ fontWeight: 500, color: 'text.secondary' }}>Class Advisor: </Box>
                  <Box component="span" sx={{ color: 'text.primary' }}>{selectedRequest.class_advisor}</Box>
                </Typography>
                <Typography variant="subtitle2">
                  <Box component="span" sx={{ fontWeight: 500, color: 'text.secondary' }}>Mentor: </Box>
                  <Box component="span" sx={{ color: 'text.primary' }}>{selectedRequest.mentor}</Box>
                </Typography>
          ...
          <DialogActions sx={{ p: 2 }}>
            {selectedRequest.approvals.principal_approval === "pending" &&
              (selectedRequest.request_type === "internship"
                ? selectedRequest.approvals.placement_officer_approval === "approved"
                : selectedRequest.approvals.hod_approval === "approved") && (
              <>
                <Button
                  color="error"
                  variant="outlined"
    ...
    <Box sx={{ maxWidth: 1450, margin: "auto", p: isMobile ? 1 : 2 }}>
      ...
      <Box sx={{ mb: 3, display: 'flex', flexDirection: isSmallMobile ? 'column' : 'row', gap: 2 }}>
        ...
        >
          ...
          <MenuItem onClick={() => handleTypeFilter("sick leave")} selected={filterType === "sick leave"}>
            <Stack direction="row" alignItems="center" spacing={1}>
          ...
          <MenuItem onClick={() => handleTypeFilter("symposium")} selected={filterType === "symposium"}>
            <Stack direction="row" alignItems="center" spacing={1}>
          ...
          <MenuItem onClick={() => handleTypeFilter("cultural fest")} selected={filterType === "cultural fest"}>
            <Stack direction="row" alignItems="center" spacing={1}>
              <EmojiEvents fontSize="small" />
              <span>Cultural Fest</span>
            </Stack>
          </MenuItem>
          <MenuItem onClick={() => handleTypeFilter("sports")} selected={filterType === "sports"}>
            <Stack direction="row" alignItems="center" spacing={1}>
              <EmojiEvents fontSize="small" />
              <span>Sports</span>
            </Stack>
          ...
          <MenuItem onClick={() => handleTypeFilter("internship")} selected={filterType === "internship"}>
            <Stack direction="row" alignItems="center" spacing={1}>
              <Work fontSize="small" />
              <span>Internship</span>
            </Stack>
          </MenuItem>
          <MenuItem onClick={() => handleTypeFilter("training")} selected={filterType === "training"}>
            <Stack direction="row" alignItems="center" spacing={1}>
        ...
        <>
          <Stack spacing={2}>
            {paginatedRequests.map((request) => {
              ...
              const overallStatus = request.approvals.principal_approval === "approved" 
                ? "approved" 
                : request.approvals.principal_approval === "rejected" 
                  ? "rejected" 
                  : "pending";
              ...
              return (
                <LeaveRequestCard key={request.request_id} status={overallStatus}>
                  <CompactAccordion 
...
export default PrincipalRequestManagement;