...
const StudentAttendanceDashboard = () => {
  ...
  const fetchStudentTableData = async (searchValue = studentTableSearch, dateValue = studentTableDate) => {
    ...
    try {
      ...
      if (attendanceFilter !== 'all') params.status = attendanceFilter;
    ...
    if (viewMode === 'daily') {
      return (
        <Grid container spacing={3} sx={{ mb: 3}}>
          ...
          <Grid item xs={12} md={6} lg={3}>
            <Card sx={{ height: '100%', borderRadius: 3, background: `linear-gradient(135deg, ${grey[500]} 0%, ${grey[600]} 100%)`, color: 'white', transition: "transform 0.3s ease-in-out, box-shadow 0.3s ease", '&:hover': { transform: 'translateY(-5px)', boxShadow: '0 10px 20px rgba(0,0,0,0.2)' } }}>
              <CardContent>
                <Typography variant="h6" gutterBottom>Early Departure</Typography>
                <Typography variant="h4" sx={{ mb: 1 }}>{earlyDeparture}</Typography>
                <Typography variant="body2">Students who left before the end of the day</Typography>
              </CardContent>
            </Card>
          </Grid>
          <Grid item xs={12} md={6} lg={3}>
            <Card sx={{ height: '100%', borderRadius: 3, background: `linear-gradient(135deg, ${grey[500]} 0%, ${grey[600]} 100%)`, color: 'white', transition: "transform 0.3s ease-in-out, box-shadow 0.3s ease", '&:hover': { transform: 'translateY(-5px)', boxShadow: '0 10px 20px rgba(0,0,0,0.2)' } }}>
              <CardContent>
                ...
                <Typography variant="h4" sx={{ mb: 1 }}>{totalStudents}</Typography>
                <Typography variant="body2">College strength</Typography>
              </CardContent>
            </Card>
          </Grid>
        </Grid>
              ...
              <Card sx={{ height: '100%', borderRadius: 3, transition: "transform 0.3s ease-in-out, box-shadow 0.3s ease", '&:hover': { transform: 'translateY(-5px)', boxShadow: '0 10px 20px rgba(0,0,0,0.2)' } }}>
                <CardContent>
                  ...
                  <Grid container spacing={1} sx={{ mb: 2 }}>
                    ...
                    <Grid item xs={6}><Typography variant="body2" color="text.secondary">Early Departure</Typography><Typography color={COLORS.earlyDeparture} fontWeight="bold">{dept.earlyDeparture}</Typography></Grid>
                  ...
                  <Box sx={{ height: 120 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={[{ name: 'Present', value: totalPresent }, { name: 'Absent', value: dept.absent }, { name: 'Late', value: dept.late }, { name: 'OD', value: dept.od }, { name: 'Internship', value: dept.internship }, { name: 'Early Departure', value: dept.earlyDeparture }]} cx="50%" cy="50%" innerRadius={30} outerRadius={50} paddingAngle={2} dataKey="value">
                          {[COLORS.present, COLORS.absent, COLORS.late, COLORS.od, COLORS.internship, COLORS.earlyDeparture].map((color, idx) => (<Cell key={`cell-${idx}`} fill={color} />))}
                        </Pie>
                      </PieChart>
                    </ResponsiveContainer>
                  </Box>
