...
const PrincipalRequestManagement = () => {
  ...
  const requestTypes = [
    ...
    { value: "exam duty", label: "Exam Duty", icon: <Description /> },
    ...
    { value: "family emergency", label: "Family Emergency", icon: <Info /> },
      ...
      if (response.data?.staff_leave_requests) {
        const formattedRequests = response.data.staff_leave_requests.map((request) => {
          const stages = [
            {
              stage: "HOD",
              status: request.approval_status.hod || "pending",
            },
            {
              stage: "Principal",
...
`${API_BASE_URL}/principal-staff-approve-request`,
        {
  ...
  const getApprovalStatus = (request) => {
    ...
    if (request.approval_status.hod === "approved") return "pending";
    return "pending";
  };
      ...
      {selectedRequest && (
        <>
          ...
          <DialogActions sx={{ p: 2 }}>
            {selectedRequest.approval_status.hod === "approved" && 
