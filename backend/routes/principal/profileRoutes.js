...
router.get('/profile/:sin_number', async (req, res) => {
  try {
    ...
    res.json({
      ...
      principal: {
        ...principal.toJSON(),
        photo: principal.photo ? principal.photo.toString('base64') : null
      }
    });
...
router.put('/update-profile/:sin_number', upload.single('photo'), async (req, res) => {
  try {
    ...
    // Get updated principal data
    const updatedPrincipal = await User.findOne({
      where: { sin_number },
      attributes: { exclude: ['password'] }
    });
    ...
    res.json({
      ...
      principal: {
        ...updatedPrincipal.toJSON(),
        photo: updatedPrincipal.photo ? updatedPrincipal.photo.toString('base64') : null
      }
    });
