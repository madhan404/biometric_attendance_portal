...
const calculateAttendance = async (startDate, endDate, mentorSinNumber, timeUnit = 'daily') => {
  ...
  const studentSinNumbers = mentees.map(student => student.sin_number);
  ...
  // Get all device logs for these mentees
...
router.get('/daily-attendance', async (req, res) => {
  try {
    ...
    console.error('Error fetching mentor daily attendance:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to fetch mentor daily attendance',
      error: error.message
    });
  }
});
...
router.get('/weekly-attendance', async (req, res) => {
  try {
    ...
    console.error('Error fetching mentor weekly attendance:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to fetch mentor weekly attendance',
      error: error.message
    });
  }
});
...
router.get('/monthly-attendance', async (req, res) => {
  try {
    ...
    console.error('Error fetching mentor monthly attendance:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to fetch mentor monthly attendance',
      error: error.message
    });
  }
});
...
router.get('/student-attendance', async (req, res) => {
  try {
    ...
    if (!sin_number) {
      return res.status(400).json({ success: false, message: 'Mentor SIN number is required' });
    }
    ...
    const mentees = await User.findAll({ where: menteeWhere });
    const sinNumbers = mentees.map(s => s.sin_number);
    ...
    // Get all device logs for these mentees in the semester
    ...
    console.error('Error fetching mentor student-level attendance:', error);
    res.status(500).json({ 
      success: false, 
      message: 'Failed to fetch mentor student-level attendance', 
      error: error.message 
    });
  }
});
